  ...
  return new Promise((resolve, reject) => {
    resolve(
      graphql(
        ...
        const paginationTemplate = path.resolve('src/workshop/index.js');
        const postsPerPage = 10;
        let numPages = Math.ceil(allPosts.length / postsPerPage);

        // Creating the main workshop index
        Array.from({ length: numPages }).forEach((_, i) => {
          createPage({
            path: i === 0 ? '/workshop' : `/workshop/${i + 1}`,
            component: paginationTemplate,
            context: {
              limit: postsPerPage,
              skip: i * postsPerPage,
              nextPage: `/workshop/${i + 2}`,
              pageNumber: i + 1,
            }
          })
        })
        ...
        let category;
        let categoryPosts;
        const categoryTemplate = path.resolve('src/workshop/category.js');
        ...
        const template = path.resolve('src/workshop/post.js');
